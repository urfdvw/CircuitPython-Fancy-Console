...
import {
    useSerialReceiveProcessor,
    body_text_to_repl_conversation,
} from "./useSerialReceiveProcessor";
...
const TextBlock = ({ info }) => {
    return <pre style={{ whiteSpace: "pre-wrap" }}>{info}</pre>;
};
...
const CodeInput = () => {
    return (
        <Box sx={widgetStyles}>
            <AceEditor mode="python" theme="tomorrow" maxLines={20} />
        </Box>
    );
};
...
export const Repl = ({ body }) => {
    ...
    return (
        <>
            ...
            {repl_conversation.waiting4code ? <CodeInput /> : <></>}
        </>
    );
};
            ...
            {sessions.map((block) => {
                return (
                    <>
                        ...
                        {block.info ? <TextBlock info={block.info} /> : <></>}
                    </>
                );
            })}
        </>
