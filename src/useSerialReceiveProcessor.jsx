...
export const output_to_sessions = (output) => {
    ...
    output = output
        .split(constants.SOFT_REBOOT)
        .slice(1)
        .join(constants.SOFT_REBOOT)
        .trimStart();
    ...
    const extra_eol = unix_line_ending
        .split(constants.TITLE_END)
        .join(constants.TITLE_END);

    // replace Done with @, so that @ will be the only indicator of session ending
    const end_unified = extra_eol
        .split(
            globStringToRegex(
                constants.TITLE_START + "*Done*" + constants.TITLE_END
            )
        )
        .join(constants.TITLE_START + "@" + constants.TITLE_END);

    // split by code running stops
    const splitted_by_ends = end_unified.split(
        globStringToRegex(constants.TITLE_START + "*@*" + constants.TITLE_END)
    );
...
export const body_text_to_repl_conversation = (body) => {
    ...
    const conversation = split_by_arrows
        .slice(1)
        .map((session) => {
            let code = session.split("\n").at(0);
            if (code.includes('exec("""')) {
                code = matchesInBetween(code, 'exec("""', '""")')[0]
                    .split("\\n")
                    .join("\n");
            }
...
export const useSerialReceiveProcessor = (output) => {
    ...
    const [history, setHistory] = useState([
        "help('modules') # list build-in modules",
    ]);
